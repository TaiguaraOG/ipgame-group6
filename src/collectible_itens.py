...
def gerar_posicoes_itens():
    posicoes = []
    margem = 50
    distancia_minima = 120
    
    while len(posicoes) < 3:
        x = random.randrange(margem, MAP_WIDTH - margem)
        y = 450
        
        posicao_valida = True
        for pos_existente in posicoes:
            if abs(x - pos_existente[0]) < distancia_minima:
